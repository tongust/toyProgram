...
main(int argc, char *argv[])
{
    ...
    /* have opened the listen fd */
    pid_t ret_pid = fork();
    if (ret_pid < 0) {
        // TODO logger
        MDebugLog("fork error") << endl;
        return 1;
    }
    ...
    if (ret_pid == 0) // child process
    {
        ...
        /* handler */
        action.sa_sigaction = [](int sig, siginfo_t *siginfo_ptr, void* ucontext) {
            /* the ppid must be static to be visiable to handler of signal */
            if (siginfo_ptr->si_pid == ppid) {
                int client_fd = siginfo_ptr->si_value.sival_int;
                if (client_fd > max_fd)
                    max_fd = client_fd;
        ...
        while (1) {
            ...
            for (auto f : client_fds) {
                //cout << "poll " << f << endl;
                if (FD_ISSET(f, &ready_set)) {
                    //cout << "connected: " << f << endl;
                    char ch1;
                    auto num_read = read(f, &ch1, 1);
                    if (num_read == 1) {
                        cout << ch1 << endl;
                    }
                    else {
    ...
    else { //                  /* Handle clients iteratively */
        union sigval sigval_clientfd;
        ...
        while (1) {
            ...
            cout << "p cfd " << cfd << endl;
            sigval_clientfd.sival_int = cfd;
            sigqueue(ret_pid, SIGINT, sigval_clientfd);
